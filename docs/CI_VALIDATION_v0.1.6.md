...
- normalize and SHA-verify the final v0.1.6 controller;
- configure/build Windows x64;
- package runtime and reconstructed source artifacts.

Retry note: Windows `git apply` materializes the patched controller with CRLF; CI now normalizes it back to canonical LF before the final controller SHA check.
